 ...
 * DashboardBackground
 *
 * Renders the cosmic background via a portal to document.body so fixed layers
 * are always viewport-relative (no ancestor transform/stacking issues).
 * Same cosmic base as Auth; constellation + shooting stars are dashboard-only.
 */
interface DashboardBackgroundProps {
  children: React.ReactNode
}

const COSMIC_BACKGROUND_ID = 'orion-dashboard-cosmic-background'

function CosmicBackgroundLayers() {
  return (
    <div id={COSMIC_BACKGROUND_ID} className="fixed inset-0 z-[-1] pointer-events-none" aria-hidden>
      {/* Base and cosmic layers (positive z so they stack inside portal) */}
      <div className="absolute inset-0 z-0 bg-background-dark" />
      <div className="absolute inset-0 z-[1] bg-cosmos opacity-40" />
      <div className="absolute inset-0 z-[2] bg-gradient-to-b from-black/80 via-black/40 to-black/90" />
      <div className="absolute inset-0 z-[3] planet-arc" />
      <div className="absolute inset-0 z-[4] crescent-glow animate-pulse-slow" />
      <div
        className="absolute inset-0 z-[5] star-field animate-spin-slow"
        style={{ animationDuration: '200s' }}
      />
      {/* Centered orbital rings */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[1200px] h-[1200px] border border-white/5 rounded-full animate-spin-slow z-[6]" />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[900px] h-[900px] border border-white/5 rounded-full animate-spin-reverse-slow z-[6]" />
      {/* Decorative layer: neon grid, orbits, constellation, telemetry, shooting stars */}
      <div className="absolute inset-0 z-[7] overflow-hidden">
        ...
        <div
          className="shooting-star z-[8]"
          style={{ top: '10%', right: '10%', animationDelay: '2s' }}
        />
        <div
          className="shooting-star z-[8]"
          style={{ top: '30%', right: '-5%', animationDelay: '7s', animationDuration: '8s' }}
        />
        <div
          className="shooting-star z-[8]"
          style={{ top: '0%', right: '40%', animationDelay: '15s' }}
        />
      </div>
    </div>
  )
...
export const DashboardBackground: React.FC<DashboardBackgroundProps> = ({ children }) => {
  const backgroundNode = <CosmicBackgroundLayers />
