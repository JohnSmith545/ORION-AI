...
export const SignupForm: React.FC<SignupFormProps> = ({ onLoginClick }) => {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Success: User created and profile updated')
    } catch (err: unknown) {
  ...
  return (
    <div className="glass-login rounded-2xl p-8 backdrop-blur-xl relative overflow-hidden group">
      ...
      <form onSubmit={handleSignup} className="flex flex-col gap-5 relative z-10">
        ...
        <div className="flex flex-col gap-3 pt-2">
          <label className="flex items-start gap-2 cursor-pointer group">
            <input
              className="mt-1 w-3.5 h-3.5 rounded border-white/20 bg-white/5 text-primary focus:ring-offset-0 focus:ring-primary/50 transition-all checked:bg-primary border-white/10"
              type="checkbox"
              required
            />
            <span className="text-[11px] text-white/40 group-hover:text-white/60 transition-colors font-light leading-snug">
              I accept the{' '}
              <a
                href="#"
        ...
        <button
          className="mt-4 w-full py-3.5 px-4 rounded-lg btn-primary-glow text-white font-display text-sm uppercase tracking-[0.15em] relative overflow-hidden group disabled:opacity-50 disabled:cursor-not-allowed"
          type="submit"
          disabled={loading}
        >
      ...
      <div className="mt-8 pt-6 border-t border-white/5 flex flex-col items-center gap-4">
        <p className="text-[10px] text-white/30 font-light">Or register via quantum link</p>
